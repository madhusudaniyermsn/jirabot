    ...
    def process_command(self, command_text: str) -> dict:
        ...
        if ("create" in doc.text or "new" in doc.text) and \
           any(t in doc.text for t in ["story", "task", "defect", "bug"]):
            ...
            # Extract summary: This is the most challenging part for simple NLP.
            # We try a few regex patterns to capture the summary text.
            ...
            if summary_match:
                ...
                # Try to capture text between issue type and project keyword
                # e.g., "create a story Implement UI in project WEBAPP"
                type_keywords = "|".join(["story", "task", "defect", "bug"])
                project_keywords = "(?:in|for|on)\\s*(?:project)?"
                # Pattern: (create/new) (a/an)? (type) (summary text) (in/for project) (project_key)
                fallback_summary_pattern = rf"(?:create|new)\s*(?:a|an)?\s*(?:{type_keywords})\s*(.+?)(?:\s*{project_keywords}\s*(\w+))?$"
                ...
                if fallback_summary_match and fallback_summary_match.group(1):
                    temp_summary = fallback_summary_match.group(1).strip()
                    # Remove common leading/trailing phrases that are not part of the summary
                    temp_summary = re.sub(r"^(?:called|titled|for)\s*", "", temp_summary, flags=re.IGNORECASE).strip()
                    # Updated regex to be more specific in removing the project key part of the command.
                    temp_summary = re.sub(r"\s*(?:in|for|on)\s*(?:project)?\s*(\b[A-Z]{2,10}\b)$", "", temp_summary, flags=re.IGNORECASE).strip()
                    parsed_data["entities"]["summary"] = temp_summary

            # Extract project key (e.g., MYPROJ, DEV, QA)
            project_match = re.search(r"(?:in|for|on)\s*(?:project)?\s*(\b[A-Z]{2,10}\b)$", command_text, re.IGNORECASE)
            if project_match:
                parsed_data["entities"]["project_key"] = project_match.group(1).upper() # Assume project keys are uppercase

            # --- NEW: Extract description from a phrase like 'with description "..."' ---
